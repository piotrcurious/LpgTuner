...
void drawWidget(Widget& w, int idx) {
  ...
  if (w.mode == MODE_DEC) {
    tft.fillRect(w.x + 55, w.y, 65, 20, 0);
    ...
    for(int r=0; r<RPM_BINS; r++) for(int l=0; l<LOAD_BINS; l++) {
      ...
      if (w.mode == MODE_HEATMAP_COMBINED) {
        ...
        if (z == LAMBDA_LEAN) tft.drawRect(mx, my, 4, 4, 0xFFFF);
        else if (z == LAMBDA_RICH) tft.fillCircle(mx+2, my+2, 2, 0xFFFF);
        else tft.fillRect(mx, my, 2, 4, 0xFFFF);
      }
      if (r == getRPMBin(currentRPM) && l == getLoadBin(currentMAP) && currentRPM > 500)
          tft.drawRect(x, y, cw-1, ch-1, 0xFFFF);
    }
  }
}
...
void loop() {
  ...
  if (millis() - lastLayoutChange >= 8000) {
    lastLayoutChange = millis(); currentLayout = (currentLayout + 1) % NUM_LAYOUTS; tft.fillScreen(0);
    for(int i=0; i<6; i++) lastUpdate[i] = 0;
  }
  static unsigned long lastUpd[3][6];
  for (int i = 0; i < 6; i++) {
    Widget& w = layouts[currentLayout][i];
    if (w.label != "" || w.mode != 0) {
      if (millis() - lastUpd[currentLayout][i] >= w.rate) {
        lastUpd[currentLayout][i] = millis(); drawWidget(w, i);
      }
    }
  }
}
