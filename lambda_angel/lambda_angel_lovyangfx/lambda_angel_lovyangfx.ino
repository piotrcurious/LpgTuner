...
Widget layouts[][6] = {
  {
    ...
    {2, "Lam:", 170, 10, 2, 100, MODE_DEC, 0, 2, {0x07FF, 120, 20}},
...
void drawWidget(Widget& w, int idx) {
  ...
  if (w.mode == MODE_DEC) {
    tft.fillRect(w.x + 60, w.y, 80, 20, 0);
    ...
    for(int r=0; r<RPM_BINS; r++) for(int l=0; l<LOAD_BINS; l++) {
      ...
      if (r == getRPMBin(currentRPM) && l == getLoadBin(currentMAP) && currentRPM > 500)
          tft.drawRect(w.x + r*cw, w.y + (LOAD_BINS-1-l)*ch, cw-1, ch-1, 0xFFFF);
    }
  }
}
...
void loop() {
  ...
  if (millis() - lastLC >= 8000) { lastLC = millis(); curL = (curL + 1) % 3; tft.fillScreen(0); }
