...
static inline void write_timing_registers(LGFX &tft, uint8_t frctr2_val, uint8_t por_fporch, uint8_t por_bporch) {
...
uint64_t BarPlotter::calibrate_bar_transfer_time(BarPlotter *self) {
    ...
    self->_tft.pushImageDMA(self->_plotX, self->_plotY, w, h, src);
    self->_tft.endWrite();
    self->_tft.waitDMA();
...
void BarPlotter::updatePdmFraction() {
    double Tt = (double)_frame_us;
    double Ts = (double)_short_duration_us;
    double Tl = (double)_long_duration_us;
    if (Tl == Ts) {
        _p_fraction = 0.0;
    } else {
        _p_fraction = (Tt - Ts) / (Tl - Ts);
    }
    if (_p_fraction < 0.0) _p_fraction = 0.0;
    if (_p_fraction > 1.0) _p_fraction = 1.0;
}
...
BarPlotter::BarPlotter(LGFX& tft) : _tft(tft),
    ...
    _even_frctr2(0x1b), _even_fporch(0x3d), _even_bporch(0x3d),
    _odd_frctr2(0x0c), _odd_fporch(0x8c), _odd_bporch(0x3f),
...
void BarPlotter::clearSpacingAreas() {
    ...
    for (size_t i = 0; i < _barCount; ++i) {
        ...
        if (_plotSpacing > 0) _tft.fillRect(gapX, _plotY, _plotSpacing, _barMaxHeight, _screenBg);
    }
}
...
//    Serial.println("RenderNextFrame");
    ...
    for (size_t i = 0; i < mid_bar_index; ++i) {
        ...
        int dx = baseX + (int)i * (w + _plotSpacing);
        int dy = baseY;
        size_t copyIndex = 0;
        ...
        } else if (_barSourceCopiesCount > 0) {
            copyIndex = i % _barSourceCopiesCount;
        }

        if (_barSourceCopies) {
            uint16_t* src = windowPtrForValueAndCopy(value, copyIndex);
            _tft.pushImageDMA(dx, dy, w, h, src);
        } else if (_dmaBarBuffer) {
            prepareDmaBarBufferForValue(value);
            _tft.pushImageDMA(dx, dy, w, h, _dmaBarBuffer);
        } else {
            if (value > 0) {
                int fillY = dy + (h - (int)value);
                _tft.fillRect(dx, fillY, w, (int)value, _colorFill);
            }
            if (value < (uint16_t)h) {
                int clearH = h - (int)value;
                _tft.fillRect(dx, dy, w, clearH, _colorFg);
            }
        }
    }
    ...
    if (mid_bar_index < (size_t)totalBars) {
        for (size_t j = mid_bar_index; j < (size_t)totalBars; ++j) {
            ...
            if (value > (uint16_t)h) value = (uint16_t)h;
            int dx = baseX + (int)j * (w + _plotSpacing);
            int dy = baseY;
            size_t copyIndex = 0;
            if (_barSourceCopiesCount >= (size_t)_patternH) {
                size_t anchor = (size_t)(_plotY % _patternH);
                ...
                copyIndex = j % _barSourceCopiesCount;
            }
...
void BarPlotter::plotTask(void* pvParameters) {
    ...
    if (self->_per_bar_us < 100) self->_per_bar_us = 100;
