...
static const float DEFAULT_RATE_HZ = 16.45f;    // default host frame target
static const uint32_t DEFAULT_PHASE_OFFSET_US = 200; // send timing command 2ms before expected frame start (tweak)
...
BarPlotter::BarPlotter(TFT_eSPI& tft) :
    ...
    _mid_offset((uint32_t)((uint64_t)(1000000.0f / DEFAULT_RATE_HZ) / 1.1)),
    _porch_step(DEFAULT_PORCH_STEP),
    _target_rate_hz(DEFAULT_RATE_HZ),
    _even_frctr2(0x1e), // for 320x240 st7789
    _even_fporch(0x49),
    _even_bporch(0x49),
    _odd_frctr2(0x1f),
    _odd_fporch(0x8d),
...
void BarPlotter::setFrameRate(float hz) {
    ...
    _mid_offset = (uint32_t)(_frame_us *0.76 ); // important fudge factor
}
...
bool BarPlotter::begin(int barCount, int barWidth, int barMaxHeight, int plotX, int plotY, int plotSpacing) {
    ...
    if (_plotTaskHandle == NULL) {
        ...
        BaseType_t ok = xTaskCreatePinnedToCore(plotTask, "plotTask", stackSize, this, 5, &_plotTaskHandle, 1);
        if (ok != pdPASS) {
...
bool BarPlotter::startBarPlot(const uint16_t* dataArray, size_t count) {
    ...
    const uint32_t default_max_wait_ms = 0; // small bounded wait for producer
    bool copied = false;
...
void BarPlotter::plotTask(void* pvParameters) {
    ...
    uint64_t last_frame_end_us = per_bar_us*(int)self->_plotDataCount; // estimate 
...
//        last_frame_start_us = esp_timer_get_time(); 
        ...
        last_frame_end_us = esp_timer_get_time();
        ...
        if (mid_bar_index >= (size_t)totalBars) {
            ...
            // this got fudge-factored here
             //       while (self->_tft.dmaBusy()) taskYIELD();

         //write_timing_registers(self->_tft, self->_odd_frctr2, self->_odd_fporch, self->_odd_bporch); // set timing to short frame 
