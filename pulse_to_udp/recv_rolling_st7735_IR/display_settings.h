...
bool odd_frame_adjusted = false; // flag to determine wheter odd frame got adjusted
                                // to avoid re-sending adjustment commands as they are sent
                                // just after it is detected that dma transaction got finished
                                // fixme - the dma transaction finished should be realized as a callback. 
bool odd_frame = false; // flag to flip if odd frame is adjusted , separate from sprSel as they need not be synchronized
