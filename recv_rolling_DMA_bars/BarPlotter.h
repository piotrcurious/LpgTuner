...
public:
    ...
    // initialize plotter (allocations, precomputes, and starts persistent plot
task)
    bool begin(int barCount, int barWidth, int barMaxHeight, int plotX, int plot
Y, int plotSpacing);

    // queue a frame of bar heights (0..barMaxHeight).
    // Returns true if the frame was accepted (queued or copied), false if dropp
ed.
    ...
    uint32_t _phase_send_offset;  // microseconds before expected frame start to
 send "start-of-frame" timing
    ...
    uint32_t _mid_offset;         // offset (us) from push time to send alternat
e timing write (default frame_us/2)
...
private:
    ...
    // pointer to active data used by plot task (may point to internal ping buff
er)
    ...
    uint16_t* _activeBuf;         // buffer currently being consumed by plot tas
k
