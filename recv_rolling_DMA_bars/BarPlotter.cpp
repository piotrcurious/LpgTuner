...
static const uint32_t DEFAULT_PHASE_OFFSET_US = 0; // send timing command 2ms be
fore expected frame start (tweak)
...
odd_frctr2, _odd_fporch, _odd_bporch);
    Serial.printf("frame_rate=%.2f Hz, frame_us=%llu us\n", _target_rate_hz, (un
signed long long)_frame_us);
...
bool BarPlotter::begin(int barCount, int barWidth, int barMaxHeight, int plotX,
int plotY, int plotSpacing) {
    _barCount = barCount;
    _barWidth = barWidth;
    ...
    if (createTallBarSourceCopiesWithPattern()) {
        Serial.printf("Using tall precomputed barSource copies with pattern (%u
copies)\n", (unsigned)_barSourceCopiesCount);
    } else {
        Serial.println("Tall precompute copies with pattern failed; attempting p
er-bar DMA buffer");
...
");
    ...
    if (!_frameBufA || !_frameBufB) {
        ...
        // still return true; fallbacks might still allow plotting with direct r
eads
    } else {
        // init
        for (size_t i=0;i<(size_t)_barCount;++i) { _frameBufA[i]=0; _frameBufB[i
]=0; }
    ...
    if (_plotTaskHandle == NULL) {
        ...
        BaseType_t ok = xTaskCreatePinnedToCore(plotTask, "plotTask", stackSize,
 this, 5, &_plotTaskHandle, 0);
        if (ok != pdPASS) {
...
bool BarPlotter::startBarPlot(const uint16_t* dataArray, size_t count) {
    ...
    if (_frameBufA && _frameBufB && _frameMutex) {
        // try to acquire quickly; block for a short bounded time to reduce fram
e drops
        if (xSemaphoreTake(_frameMutex, pdMS_TO_TICKS(default_max_wait_ms)) == p
dTRUE) {
            // copy into queuedBuf
        ...
        // No internal buffers: fallback to original pointer-swap behavior (dang
erous but compat)
        // We will simply set the pointer and let plot task access it. This is l
ess safe; prefer buffers.
...
bool BarPlotter::createTallBarSourceCopiesWithPattern() {
    ...
    Serial.printf("Precompute (tall copies with pattern, for static pattern): al
locating %u bytes for %u pixels (w=%u, rowsPerCopy=%u, copies=%u)\n",
                  (unsigned)totalBytes, (unsigned)totalPixels, (unsigned)w, (uns
igned)rowsPerCopy, (unsigned)copies);

    _barSourceCopies = (uint16_t*) heap_caps_malloc(totalBytes, MALLOC_CAP_DMA);
    if (!_barSourceCopies) {
        Serial.println("Precompute copies: allocation failed (no DMA-capable mem
ory)");
        return false;
    }
...
uint16_t* BarPlotter::windowPtrForValueAndCopy(uint16_t v, size_t copyIndex) {
    ...
    return _barSourceCopies + (copyIndex * _barSourcePixelsSingle) + ((size_t)v
* _rowWidth);
}
...
//uint64_t calibrate_bar_transfer_time(BarPlotter *self) {
    ...
    if (self->_barSourceCopies && self->_barSourceCopiesCount > 0) {
        ...
        // no DMA buffer available; create a tiny local DMA-capable buffer for c
alibration
...
nsigned long long)per_bar_us);

    uint64_t last_frame_start_us = esp_timer_get_time();

    while (1) {
        // continuous task: wait for either queued frame or for scheduled next f
rame slot
        // If no frames queued, we still run to keep timing stable.
        // compute schedule next frame start without depending on external trigg
er
        ...
        // If a queued frame exists in ping-pong buffers, swap it in for consump
tion
        if (self->_frameBufA && self->_frameBufB && self->_frameMutex) {
            if (self->_hasQueuedFrame) {
                // fast swap under mutex
                if (xSemaphoreTake(self->_frameMutex, pdMS_TO_TICKS(5)) == pdTRU
E) {
            ...
            // fallback: if no internal buffers, plotter uses pointer provided b
y startBarPlot
...
;
        ...
        int64_t wake_at = (int64_t)next_frame_deadline - (int64_t)self->_phase_s
end_offset;
        ...
        // ----------------- Stage A: start-of-frame timing + push bars before m
id index
        self->_tft.startWrite();
        if (use_even_set) {
            write_timing_registers(self->_tft, self->_even_frctr2, self->_even_f
porch, self->_even_bporch);
        } else {
            write_timing_registers(self->_tft, self->_odd_frctr2, self->_odd_fpo
rch, self->_odd_bporch);
        }
...
;
                ...
                self->_tft.pushImageDMA(dx, dy, w, h, self->_dmaBarBuffer, nullp
tr);
            } else {
                if (value > 0) {
                    int fillY = dy + (h - (int)value);
                    self->_tft.fillRect(dx, fillY, w, (int)value, self->_colorFi
ll);
                }
                if (value < (uint16_t)h) {
        ...
        // ----------------- Stage B: wait for last DMA to finish and send mid-f
rame timing
        if (mid_bar_index >= (size_t)totalBars) {
            // All bars already pushed in Stage A. Wait for DMA and then send mi
d (late).
            while (self->_tft.dmaBusy()) taskYIELD();
            self->_tft.startWrite();
            if (use_even_set) {
                write_timing_registers(self->_tft, self->_odd_frctr2, self->_odd
_fporch, self->_odd_bporch);
            } else {
                write_timing_registers(self->_tft, self->_even_frctr2, self->_ev
en_fporch, self->_even_bporch);
            }
            self->_tft.endWrite();
            // nothing left to push
        } else {
            // Need to send mid between two chunks: wait for current DMA to fini
sh
            while (self->_tft.dmaBusy()) taskYIELD();
...
en_fporch, self->_even_bporch);
            ...
            // ----------------- Stage C: push remaining bars [mid_bar_index ..
totalBars-1]
            self->_tft.startWrite();
            for (size_t j = mid_bar_index; j < (size_t)totalBars; ++j) {
                ...
                if (self->_barSourceCopiesCount >= (size_t)self->_patternH) {
                    size_t anchor = (size_t)(self->_plotY % self->_patternH);
                    size_t v_mod = (size_t)(value % (uint16_t)self->_patternH);
                    copyIndex = (anchor + self->_patternH - v_mod) % self->_patt
ernH;
                } else if (self->_barSourceCopiesCount > 0) {
                ...
                if (self->_barSourceCopies) {
                    uint16_t* src = self->windowPtrForValueAndCopy(value, copyIn
dex);
                    self->_tft.pushImageDMA(dx, dy, w, h, src, nullptr);
                } else if (self->_dmaBarBuffer) {
                    self->prepareDmaBarBufferForValue(value);
                    self->_tft.pushImageDMA(dx, dy, w, h, self->_dmaBarBuffer, n
ullptr);
                } else {
                    if (value > 0) {
                        int fillY = dy + (h - (int)value);
                        self->_tft.fillRect(dx, fillY, w, (int)value, self->_col
orFill);
                    }
                    if (value < (uint16_t)h) {
                        int clearH = h - (int)value;
        ...
        self->_plotInProgress = false; // single frame consumed; still task runs
 continuously
