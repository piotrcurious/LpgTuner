...
uint16_t* sprPtr[3];  // Direct pointers to sprite buffers
...
SemaphoreHandle_t bufferMutex; // Semaphore for synchronization

void drawGraphics(void* parameter) {
  while (true) {
    // Wait for access to drawing buffer
    if (xSemaphoreTake(bufferMutex, portMAX_DELAY)) {
      ...
      xSemaphoreGive(bufferMutex); // Release semaphore
    }
...
void setup() {
  ...
  // Create a mutex for buffer swapping
...
void loop() {
  ...
  if (current_time - last_display_time >= 1000 / DISPLAY_REFRESH_RATE) {
    ...
    // Push the new buffer to the display
